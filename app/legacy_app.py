...
async def improve_bullet(payload: ImproveBulletPayload):
    ...
    try:
        ...
        tone_instruction = tone_instructions.get(
            payload.tone, tone_instructions["professional"]
        )
...
Return ONLY the bullet points, one per line, no numbering or explanations."""
        ...
        bullet_points = [
            line.strip() for line in bullet_points_text.split("\n") if line.strip()
        ]
...
async def generate_summary(payload: GenerateSummaryPayload):
    ...
    try:
        achievements = (
            f"Key achievements: {payload.achievements}" if payload.achievements else ""
        )
        ...
        response = requests.post(
            ...
            json={
                ...
                "messages": [
                    {
                        "role": "system",
                        "content": "You are a professional resume writer. Rewrite content to be concise, metric-driven, and keyword optimized without sounding artificial.",
                    },
                    {"role": "user", "content": prompt},
                ],
                "max_tokens": 220,
...
Example format: ["Implemented scalable microservices architecture using Kubernetes and Docker", "Optimized CI/CD pipelines with Jenkins reducing deployment time by 40%", "Developed REST APIs using Python and Flask handling 10K+ requests daily"]"""
        ...
        data = {
            ...
            "messages": [
                {
                    "role": "system",
                    "content": "You are a professional resume writer. Create compelling bullet points that highlight achievements and technical skills while naturally incorporating keywords.",
                },
        ...
        try:
            ...
            bullets = [
                line.strip().replace("•", "").replace("*", "").replace("-", "").strip()
                for line in bullets_text.split("\n")
...
Return as JSON array: [{{"category": "Technical Skills", "suggestion": "Add Python and SQL to skills section and include in work experience bullets"}}, ...]"""
                ...
                if response.status_code == 200:
                    result = response.json()
                    suggestions_text = result["choices"][0]["message"][
                        "content"
                    ].strip()
        ...
        if user_email and db:
            try:
                ...
                if user:
                    ...
                    logger.info(
                        f"Job match analytics tracked for user {user_email}: score {similarity_result.get('similarity_score', 0)}"
                    )
            except Exception as e:
...
async def generate_cover_letter(payload: CoverLetterPayload):
    ...
    try:
        ...
        custom_context = (
            f"\nAdditional Requirements: {payload.custom_requirements}"
    ...
    try:
        ...
        try:
            result = subprocess.run(
                ["antiword", temp_file_path], capture_output=True, text=True, timeout=30
            )
...
async def export_pdf(
    payload: ExportPayload, user_email: str = None, db: Session = Depends(get_db)
):
    try:
...
async def upload_resume(file: UploadFile = File(...)):
    try:
        ...
        logger.info(
            f"Upload: filename={filename}, type={file_type}, content_type={file.content_type}, size={len(contents)}"
        )
        ...
        if (
            ...
            or "wordprocessingml" in str(file.content_type)
        ):
...
async def get_enhanced_ats_score(payload: EnhancedATSPayload):
    ...
    try:
        ...
        result = enhanced_ats_checker.get_enhanced_ats_score(
            resume_data, payload.job_description
        )
...
Return ONLY the updated resume JSON, no explanations."""
                    ...
                    if response.status_code == 200:
                        result = response.json()
                        improved_content = result["choices"][0]["message"][
                            "content"
                        ].strip()
                        ...
                        try:
                            ...
                            logger.warning(
                                f"Could not parse improved resume for: {improvement.get('title', '')}"
                            )
                            continue
                    else:
                        logger.warning(
                            f"OpenAI API error for improvement: {improvement.get('title', '')}"
                        )
                        continue
        ...
        new_result = enhanced_ats_checker.get_enhanced_ats_score(
            improved_resume, payload.job_description
        )
                ...
                if openai_client:
                    try:
                        ...
                        if response.status_code == 200:
                            ...
                            improved_content = ai_response["choices"][0]["message"][
                                "content"
                            ]
    ...
    try:
        ...
        data = {
            ...
            "messages": [
                {
                    "role": "system",
                    "content": "You are a professional resume writer and career coach. Generate high-quality, ATS-friendly resume content.",
                },
                {"role": "user", "content": prompt},
            ],
            "max_tokens": 800,
        ...
        try:
            ...
            return {
                "success": True,
                "companyName": result.get(
                    "companyName", payload.currentCompany or "Tech Company"
                ),
                "jobTitle": result.get(
                    "jobTitle", payload.currentJobTitle or "Software Engineer"
                ),
                "dateRange": result.get(
                    "dateRange", payload.currentDateRange or "2020-2023"
                ),
                "bullets": result.get(
                    "bullets",
    ...
    try:
        ...
        for resume in resumes:
            ...
            match_sessions = (
                ...
                .filter(MatchSession.resume_id == resume.id)
                ...
                recent_matches_data.append(
                    {
                        ...
                        "resume_version_id": (
                            match_version.id if match_version else None
                        ),
                        "resume_version_number": (
                            match_version.version_number if match_version else None
                        ),
                        "created_at": (
                            ms.created_at.isoformat() if ms.created_at else None
                        ),
                    }
                )
...
async def delete_resume(
    ...
    try:
        ...
        if not resume:
            logger.warning(
                f"delete_resume: Resume {resume_id} not found or doesn't belong to user {user_email}"
            )
            raise HTTPException(
        ...
        match_sessions = (
            db.query(MatchSession).filter(MatchSession.resume_id == resume_id).all()
        )
        for ms in match_sessions:
            db.delete(ms)
        ...
        shared_resumes = (
            db.query(SharedResume).filter(SharedResume.resume_id == resume_id).all()
        )
        for sr in shared_resumes:
            db.delete(sr)
        logger.info(
            f"delete_resume: Deleted {len(shared_resumes)} shared resume records"
        )

        # Delete export analytics
        export_analytics = (
            db.query(ExportAnalytics)
            .filter(ExportAnalytics.resume_id == resume_id)
            .all()
        )
        for ea in export_analytics:
            db.delete(ea)
        logger.info(
            f"delete_resume: Deleted {len(export_analytics)} export analytics records"
        )
        ...
        logger.error(
            f"delete_resume: Error deleting resume {resume_id}: {e}", exc_info=True
        )
        db.rollback()
        raise HTTPException(
...
async def get_resume_versions(
    resume_id: int, user_email: str, db: Session = Depends(get_db)
):
...
async def rollback_version(
    payload: RollbackVersionPayload, user_email: str, db: Session = Depends(get_db)
):
...
async def compare_versions(
    payload: CompareVersionsPayload, user_email: str, db: Session = Depends(get_db)
):
...
async def delete_version(
    version_id: int, user_email: str, db: Session = Depends(get_db)
):
...
async def create_shared_resume(
    ...
    try:
        ...
        logger.info(
            f"Creating SharedResume record with resume_id={resume_id}, user_id={user.id}"
        )
        ...
        if not base_url or base_url == "http://localhost:3000":
            logger.warning(
                "FRONTEND_URL not set or using localhost - this will break in production!"
            )
...
async def get_shared_resume(
    share_token: str, password: str = None, db: Session = Depends(get_db)
):
...
async def get_shared_resume_analytics(
    share_token: str, user_email: str, db: Session = Depends(get_db)
):
    ...
    try:
        ...
        shared_resume = (
            db.query(SharedResume)
            .filter(
                SharedResume.share_token == share_token, SharedResume.user_id == user.id
            )
            .first()
        )
        ...
        views = (
            ...
            .order_by(ResumeView.created_at.desc())
            .all()
        )
...
async def deactivate_shared_resume(
    share_token: str, user_email: str, db: Session = Depends(get_db)
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to deactivate shared resume"
        )
...
async def add_shared_resume_comment(
    share_token: str, payload: SharedResumeCommentPayload, db: Session = Depends(get_db)
):
...
async def get_job_match_details(
    match_id: int, user_email: str, db: Session = Depends(get_db)
):
    ...
    try:
        ...
        job_match = (
            db.query(JobMatch)
            .filter(JobMatch.id == match_id, JobMatch.user_id == user.id)
            .first()
        )
...
class MatchCreate(BaseModel):
    ...
    jd_metadata: Optional[Dict[str, Any]] = (
        None  # JD metadata (easy_apply_url, work_type, job_type, company)
    )
...
def create_or_update_job_description(
    ...
    try:
        ...
        final_title = _determine_final_job_title(
            payload.title, payload.content, extracted_dict
        )
        ...
        if not new_columns_exist:
            ...
            if jd and jd.id:
                ...
                try:
                    ...
                    db.commit()
                    logger.info(
                        f"Successfully updated job description {jd.id} using fallback method (columns don't exist yet)"
                    )
                    return {
                        "id": jd.id,
                        "message": "saved",
            ...
            try:
                ...
                db.commit()
                logger.info(
                    f"Successfully saved job description {jd_id} using fallback method (columns don't exist yet)"
                )
                return {
                    "id": jd_id,
                    "message": "saved",
        ...
        jd.max_salary = (
            getattr(payload, "max_salary", None)
            if hasattr(payload, "max_salary")
            else None
        )
        jd.status = (
            getattr(payload, "status", "bookmarked")
            if hasattr(payload, "status")
            else "bookmarked"
        )
        jd.follow_up_date = (
            getattr(payload, "follow_up_date", None)
            if hasattr(payload, "follow_up_date")
            else None
        )
        jd.important_emoji = (
            getattr(payload, "important_emoji", None)
            if hasattr(payload, "important_emoji")
            else None
        )
        jd.notes = (
            getattr(payload, "notes", None) if hasattr(payload, "notes") else None
        )
...
def get_job_description(jd_id: int, db: Session = Depends(get_db)):
    ...
    matches = (
        db.query(MatchSession)
        .filter(MatchSession.job_description_id == jd.id)
        .order_by(MatchSession.created_at.desc())
        .all()
    )
    ...
    for link in jd.resume_versions or []:
        ...
        if not best_resume_version or (data["ats_score"] or 0) > (
            best_resume_version["ats_score"] or 0
        ):
    ...
    return {
        ...
        "follow_up_date": (lambda d: d.isoformat() if d else None)(
            getattr(jd, "follow_up_date", None)
        ),
...
def create_or_update_job_description_api(
    ...
    try:
        logger.info(
            f"create_or_update_job_description_api: Received request with title={payload.title}, company={payload.company}"
        )
        ...
        if authorization and authorization.startswith("Bearer "):
            ...
            logger.info(
                f"create_or_update_job_description_api: Received token (not validated yet)"
            )
        ...
        logger.info(
            f"create_or_update_job_description_api: Successfully saved job description with id={result.get('id')}"
        )
        return result
    except HTTPException:
        raise
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to save job description: {str(e)}"
        )


@app.get("/api/job-descriptions")
def list_job_descriptions_api(
    user_email: Optional[str] = None, db: Session = Depends(get_db)
):
    try:
        ...
        logger.exception(
            f"Failed to list job descriptions via API (user_email: {user_email})",
            exc_info=True,
        )
        raise HTTPException(status_code=500, detail=str(e))
...
def delete_job_description_api(
    jd_id: int, user_email: Optional[str] = Query(None), db: Session = Depends(get_db)
):
    ...
    try:
        ...
        if user_email:
            ...
            if user and jd.user_id and jd.user_id != user.id:
                logger.warning(
                    f"delete_job_description_api: User {user_email} doesn't own JD {jd_id}"
                )
                raise HTTPException(
                    status_code=403,
        ...
        db.query(JobResumeVersion).filter(
            JobResumeVersion.job_description_id == jd_id
        ).delete()
        db.query(JobCoverLetter).filter(
        ...
        if jd_has_new_columns:
            ...
            # Use raw SQL delete
            db.execute(
                text("DELETE FROM job_descriptions WHERE id = :jd_id"), {"jd_id": jd_id}
            )
        ...
        logger.error(
            f"delete_job_description_api: Error deleting JD {jd_id}: {e}", exc_info=True
        )
        db.rollback()
        raise HTTPException(
            status_code=500, detail=f"Failed to delete job description: {str(e)}"
        )


@app.get("/api/job-descriptions/{jd_id}")
def get_job_description_api(
    jd_id: int, user_email: Optional[str] = None, db: Session = Depends(get_db)
):
