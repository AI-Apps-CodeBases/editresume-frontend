...
class GrammarStyleChecker:
    ...
    def check_action_verbs(self, text: str) -> Tuple[int, List[StyleIssue]]:
        ...
        strong_verbs = [
            ...
            'created', 'delivered', 'developed', 'executed', 'generated', 'implemented',
            'improved', 'increased', 'launched', 'led', 'managed', 'optimized',
            'produced', 'reduced', 'streamlined', 'transformed', 'utilized'
        ]
    ...
    def calculate_style_score(self, text: str) -> StyleScore:
        ...
        all_issues = grammar_issues + passive_issues + weak_verb_issues + readability_issues + strength_issues
        ...
        if passive_issues:
            suggestions.append(f"Convert {len(passive_issues)} passive voice sentences to active")
        if weak_verb_issues:
            suggestions.append(f"Replace {len(weak_verb_issues)} weak verbs with stronger alternatives")
    ...
    def get_improvement_suggestions(self, text: str) -> List[str]:
        ...
        if len(text) > 1000:
            suggestions.append("Consider shortening the content - resumes should be concise")
        
        if text.count('.') / len(text.split()) > 0.3:
            suggestions.append("Too many periods - consider using bullet points instead")
        
        if 'I ' in text or ' my ' in text or ' me ' in text:
            suggestions.append("Remove first-person pronouns - use third person or action verbs")
        
        if text.count('responsibilities') > 0:
            suggestions.append("Replace 'responsibilities' with 'achievements' or specific results")
