...
class EnhancedATSChecker:
    def __init__(self):
        ...
        self.ats_keywords = {
            ...
            'technical_terms': [
                ...
                'microservices', 'kubernetes', 'docker', 'aws', 'azure', 'gcp', 'devops',
                'ci/cd', 'agile', 'scrum', 'machine learning', 'ai', 'data science'
            ],
            'metrics': [
                'percent', '%', 'increase', 'decrease', 'reduction', 'improvement',
                'efficiency', 'performance', 'cost', 'revenue', 'profit', 'roi',
                'kpi', 'metrics', 'analytics', 'growth', 'scale', 'budget'
            ],
            'leadership': [
                'team', 'lead', 'manage', 'mentor', 'guide', 'direct', 'supervise',
                'coordinate', 'facilitate', 'collaborate', 'strategic', 'vision',
                'initiative', 'champion', 'advocate', 'influence'
            ]
        }
        
        # Industry-specific keywords
        self.industry_keywords = {
            'tech': ['software', 'development', 'programming', 'coding', 'engineering', 'technical'],
            'finance': ['financial', 'banking', 'investment', 'trading', 'risk', 'compliance'],
            'healthcare': ['medical', 'clinical', 'patient', 'health', 'treatment', 'diagnosis'],
            'marketing': ['marketing', 'brand', 'campaign', 'digital', 'social', 'content'],
            'sales': ['sales', 'revenue', 'client', 'customer', 'business development', 'account']
        }
    ...
    def analyze_resume_structure(self, resume_data: Dict) -> Dict[str, Any]:
        ...
        return {
            ...
            'has_contact_info': bool(resume_data.get('email') or resume_data.get('phone'))
        }

    def analyze_keyword_optimization(self, resume_text: str, job_description: str = None) -> Dict[str, Any]:
        ...
        leadership_count = sum(1 for word in words if word in self.ats_keywords['leadership'])
        ...
        leadership_density = (leadership_count / total_words) * 100 if total_words > 0 else 0
        ...
        if job_description:
            ...
            job_keywords = [word.strip('.,!?;:"()[]{}') for word in job_words if word.isalpha()]
            matching_keywords = set(words) & set(job_keywords)
            job_match_score = (len(matching_keywords) / len(set(job_keywords))) * 100 if job_keywords else 0
            
            missing_job_keywords = set(job_keywords) - set(words)
            if missing_job_keywords:
                job_suggestions.append(f"Add these job-relevant keywords: {', '.join(list(missing_job_keywords)[:5])}")
        ...
        keyword_score = min(100, base_score + action_bonus + technical_bonus + metrics_bonus + leadership_bonus + job_bonus)
    ...
    def analyze_content_quality(self, resume_data: Dict) -> Dict[str, Any]:
        ...
        vague_terms = ['responsible for', 'assisted with', 'helped with', 'involved in', 'participated in']
        vague_count = sum(1 for term in vague_terms if term.lower() in text_content.lower())
        ...
        strong_verbs = ['achieved', 'delivered', 'increased', 'reduced', 'improved', 'optimized', 'transformed']
        strong_verb_count = sum(1 for verb in strong_verbs if verb.lower() in text_content.lower())
        
        # Check for buzzwords (can be good or bad)
        buzzwords = ['synergy', 'leverage', 'paradigm', 'disrupt', 'innovative', 'cutting-edge']
        buzzword_count = sum(1 for word in buzzwords if word.lower() in text_content.lower())
    ...
    def check_formatting_compatibility(self, resume_data: Dict) -> Dict[str, Any]:
        ...
        if sections:
            bullet_consistency = all(
                any('•' in bullet.get('text', '') or '-' in bullet.get('text', '') for bullet in section.get('bullets', []))
                for section in sections if section.get('bullets')
            )
    ...
    def generate_ai_improvements(self, resume_data: Dict, job_description: str = None) -> List[ATSImprovement]:
        ...
        sections = resume_data.get('sections', [])
        experience_section = next((s for s in sections if 'experience' in s.get('title', '').lower()), None)
        ...
        skills_section = next((s for s in sections if 'skill' in s.get('title', '').lower()), None)
        ...
        leadership_count = sum(1 for word in leadership_keywords if word.lower() in text_content.lower())
    ...
    def calculate_comprehensive_score(self, resume_data: Dict, job_description: str = None) -> Dict[str, Any]:
        ...
        keyword_analysis = self.analyze_keyword_optimization(resume_text, job_description)
        ...
        if structure_analysis['missing_sections']:
            all_suggestions.append(f"Add missing sections: {', '.join(structure_analysis['missing_sections'])}")
    ...
    def get_enhanced_ats_score(self, resume_data: Dict, job_description: str = None) -> Dict[str, Any]:
