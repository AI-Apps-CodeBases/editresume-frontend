  ...
  return (
    ...
    >
      <div className="flex items-start justify-between mb-3">
        ...
        {isSelected && (
          <span className="text-purple-600 text-sm font-medium ml-2">✓ Selected</span>
        )}
      </div>
      
      <div className="mb-3 p-2 bg-gray-50 rounded border border-gray-200 overflow-hidden" style={{ height: '200px' }}>
        <TemplatePreview template={template} resumeData={resumeData} scale={0.25} />
      </div>
      
      <div className="flex items-center gap-4 text-xs text-gray-500">
        <span className="capitalize px-2 py-1 bg-gray-100 rounded">{template.category}</span>
        <span className="px-2 py-1 bg-blue-100 rounded text-blue-700">ATS: {template.atsScore}%</span>
      </div>
    </button>
  )
}
