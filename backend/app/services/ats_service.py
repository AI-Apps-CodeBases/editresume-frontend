...
class ATSChecker:
    ...
    def analyze_keyword_density(self, resume_text: str) -> Dict[str, any]:
        ...
        keyword_score = min(
            100, base_score + action_bonus + technical_bonus + metrics_bonus
        )
    ...
    def calculate_overall_score(self, resume_data: Dict) -> Dict[str, any]:
        ...
        overall_score = (
            section_score * 0.4 + keyword_score * 0.3 + formatting_score * 0.3
        )
        ...
        if section_analysis["missing_sections"]:
            all_suggestions.append(
                f"Add missing sections: {', '.join(section_analysis['missing_sections'])}"
            )
