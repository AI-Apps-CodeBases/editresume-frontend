...
export function ResumeSelectionModal({
  ...
  const remainingSlots = useMemo(
    () => Math.max(maxSelectable - selectedIds.length, 0),
    [maxSelectable, selectedIds.length]
  )
        ...
        <header className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
          <div>
            ...
            <p className="text-sm text-slate-500">
              Pick up to {maxSelectable} resumes. We’ll blend the best parts from each.
            </p>
          </div>
          <button
            type="button"
          ...
          {loading ? (
            ...
            <div className="rounded-2xl border border-dashed border-slate-200 p-10 text-center">
              ...
              <p className="mt-2 text-sm text-slate-500">
                Save a resume in the editor to use it as a source for automation.
              </p>
              {onRefresh && (
                <button
