...
import { JobManualEntryPanel } from './JobManualEntryPanel'
      ...
      >
        <header className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
          <div>
            ...
            <h2 className="text-lg font-semibold text-slate-900">Tailor to a job description</h2>
            <p className="text-sm text-slate-500">Pick a saved job or paste a new JD to extract high-density keywords.</p>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="grid flex-1 grid-cols-1 gap-6 overflow-y-auto px-6 py-4 lg:grid-cols-2">
          <div className="flex flex-col gap-4">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-semibold uppercase tracking-[0.25em] text-slate-500">Saved jobs</h3>
              {onRefresh && (
                <button
            ...
            {loading ? (
              ...
              <div className="flex flex-1 flex-col items-center justify-center rounded-2xl border border-dashed border-slate-200 p-10 text-center">
                ...
                <p className="mt-2 text-sm text-slate-500">Capture a job description from LinkedIn or paste one manually to begin.</p>
              </div>
            ) : (
                      ...
                      {job.skills.length > 0 && (
                        <div className="mt-3 flex flex-wrap gap-2">
                          {job.skills.slice(0, 6).map((skill) => (
                            <span key={`${job.id}-${skill}`} className="rounded-full bg-slate-100 px-2 py-1 text-xs font-medium text-slate-600">
                              {skill}
                            </span>
                          ))}
                        </div>
        ...
        <footer className="flex items-center justify-between gap-4 border-t border-slate-200 px-6 py-4">
          <div className="text-xs text-slate-400">Use a saved job or paste a new one to start tailoring your resume.</div>
